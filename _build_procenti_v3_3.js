...
const READER_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v3.3 — Проценты, пропорции, масштаб — reader.html';
const PREVIEW_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v3.3 — Проценты, пропорции, масштаб — preview.html';
...
function buildReader() {
  let nav = '<h1 style="color:#3E2723;font-weight:700;letter-spacing:-0.01em;">Чистовые слайды v3.3 — Проценты, пропорции, масштаб</h1>';
  ...
  for (let s = 0; s < slides.length; s++) {
    ...
    if (nextSlideH2 !== curH2 && nextSlideH2 !== -1) {
      const nextH2 = curH2 + 1;
      if (bridgeByIdx.has(nextH2)) {
        const b = bridgeByIdx.get(nextH2);
        const lastSlideNum = slides[s].num.replace('.', '-');
        main += renderBridge(b, 'bridge-after-' + lastSlideNum);
      }
    }
  }
...
function buildPreview() {
  let html = '<h1>Чистовые слайды v3.1 — Проценты, пропорции, масштаб — обзор</h1>';
  ...
  for (let i = 0; i < blockInfo.length; i++) {
    ...
    for (const slide of sIn) html += '<li style="margin-bottom:4px;"><a href="Чистовые слайды v3.3 — Проценты, пропорции, масштаб — reader.html#slide-' + slide.num + '" style="color:' + color + ';text-decoration:none;">' + slide.num + '. ' + escHtml(slide.title) + '</a></li>';
    html += '</ol></details>';
  }
