// Парсер v4.6 → reader.html + preview.html
const fs = require('fs');

const MD_PATH = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v4.6 — Множества, НОК, НОД.md';
const READER_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v4.6 — Множества, НОК, НОД — reader.html';
const PREVIEW_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v4.6 — Множества, НОК, НОД — preview.html';
...
function buildReader() {
  let nav = '<h1>Чистовые слайды v4.6 — Множества, НОК, НОД</h1>';
...
function buildPreview() {
  ...
  for (let i = 0; i < blockInfo.length; i++) {
    ...
    for (const slide of sIn) html += '<li style="margin-bottom:4px;"><a href="Чистовые слайды v4.6 — Множества, НОК, НОД — reader.html#slide-' + slide.num + '" style="color:' + color + ';text-decoration:none;">' + slide.num + '. ' + escHtml(slide.title) + '</a></li>';
    html += '</ol></details>';
  }
  return '<!DOCTYPE html>\n<html lang="ru">\n<head>\n<meta charset="UTF-8">\n<meta name="viewport" content="width=device-width, initial-scale=1.0">\n<title>Чистовые слайды v4.6 — Множества, НОК, НОД — обзор</title>\n<style>\nbody { font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif; max-width:900px; margin:0 auto; padding:32px 24px; background:#f7fafc; color:#1a202c; }\nh1 { margin:0 0 8px 0; }\n</style>\n</head>\n<body>\n' + html + '\n</body>\n</html>\n';
}
