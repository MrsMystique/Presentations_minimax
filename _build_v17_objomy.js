// Парсер → reader.html + preview.html для партии v17 Объёмы многогранников
// Структура: ## → слайд (с автонумерацией), # → блок/дополнение
const fs = require('fs');

const TITLE = 'v17 — Объёмы многогранников';
const MD_PATH = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\v17 — Объёмы многогранников.md';
const READER_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\v17 — Объёмы многогранников — reader.html';
const PREVIEW_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\v17 — Объёмы многогранников — preview.html';
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith('|') && line.trim().endsWith('|')) {
      ...
      if (tableLines.length >= 2) {
        ...
        // Single line - treat as paragraph
        out.push('<p>' + inline(escHtml(line)) + '</p>');
      }
      continue;
    }
    // Paragraph: gather lines until we hit a recognized block element
    const pLines = [];
    while (i < lines.length && lines[i].trim() !== '') {
      const cur = lines[i];
      // Stop if we hit a header, blockquote, list (top-level), or table
      if (/^####|^###/.test(cur)) break;
    ...
    if (pLines.length > 0) {
      ...
      // Safety net - shouldn't happen but prevent infinite loop
      i++;
    }
  }
  return out;
...
nav.toc .addition-header { display:block; margin-top:14px; margin-bottom:4px; font-weight:700; font-size:13px; color:#5D7BAB; text-transform:uppercase; letter-spacing:0.05em; }
...
function buildReader() {
  ...
  for (let i = 0; i < blockInfo.length; i++) {
    ...
    else if (b.kind === 'addition') nav += '<a class="addition-header">Дополнение. ' + escHtml(b.name) + '</a>';
    else if (b.kind === 'bridge') nav += '<a class="bridge-header">' + escHtml(b.name) + '</a>';
