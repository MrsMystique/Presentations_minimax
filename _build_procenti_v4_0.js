// Парсер v3 → reader.html + preview.html (с поддержкой картинок)
const fs = require('fs');

const VERSION = '5.0';
const MD_PATH = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v' + VERSION + ' — Проценты, пропорции, масштаб.md';
const READER_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v' + VERSION + ' — Проценты, пропорции, масштаб — reader.html';
const PREVIEW_HTML = 'C:\\Users\\admin\\Desktop\\математика мои слайды\\Minimax\\Чистовые слайды v' + VERSION + ' — Проценты, пропорции, масштаб — preview.html';
...
function buildReader() {
  let nav = '<h1 style="color:#3E2723;font-weight:700;letter-spacing:-0.01em;">Чистовые слайды v' + VERSION + ' — Проценты, пропорции, масштаб</h1>';
...
function buildPreview() {
  ...
  for (let i = 0; i < blockInfo.length; i++) {
    ...
    for (const slide of sIn) html += '<li style="margin-bottom:4px;"><a href="Чистовые слайды v' + VERSION + ' — Проценты, пропорции, масштаб — reader.html#slide-' + slide.num + '" style="color:' + color + ';text-decoration:none;">' + slide.num + '. ' + escHtml(slide.title) + '</a></li>';
    html += '</ol></details>';
  }
  return '<!DOCTYPE html>\n<html lang="ru">\n<head>\n<meta charset="UTF-8">\n<meta name="viewport" content="width=device-width, initial-scale=1.0">\n<title>Чистовые слайды v' + VERSION + ' — Проценты, пропорции, масштаб — обзор</title>\n<style>\nbody { font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif; max-width:900px; margin:0 auto; padding:32px 24px; background:#F5F2EB; color:#3E2723; background-image:linear-gradient(180deg, rgba(245,242,235,1) 0%, rgba(240,235,225,1) 100%); }\nh1 { margin:0 0 8px 0; }\n</style>\n</head>\n<body>\n' + html + '\n</body>\n</html>\n';
}
